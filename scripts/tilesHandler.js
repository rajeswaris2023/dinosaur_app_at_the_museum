...
function createHumanTileDiv(humanPicturePath) {
    let divElement = createDivElement('tile-div');
    ...
    divElement.appendChild(pictureDivElement);
    ...
    divElement.appendChild(linkDivElement);
...
function createTilesPage(dinosaurFacts, humanPicturePath) {
    setPageBodyStyle();
    let tilesDiv = createDivElement('tiles');

    for(let index = 0; index < dinosaurFacts.length; index++) {
        if(index === 4) {
            const humanDiv = createHumanTileDiv(humanPicturePath);
